...
commentSchema.pre('update', function(next) {
  const comment = this;
  console.log("commentSchema update pre");
});
commentSchema.pre('findOneAndUpdate', function(next) {
  const comment = this;
  console.log("commentSchema findOneAndUpdate pre");
});
...
threadSchema.pre('findOneAndUpdate', function(next) {
  ...
  console.log(thread);
  var id = thread._update.$push.comments._id;

  /*
  bcrypt.genSalt(10, function(err, salt) {
    if (err) { console.log(err);  }
    // 솔트 추가해서 해시 돌리고
    bcrypt.hash(id.toString(), salt, function(error, hash) {
        if (error) { console.log(error); }
        // 비번은 해시값으로
        console.log(hash);
        thread._update.$push.comments.seq_id = hash.replace(/[^0-9]/g,'');
        console.log(thread._update);
        //thread.comments.findOneAndUpdate({}, {'seq_id': hash}, () => {});
        
      });
  });
  */
  var hashString = SHA256(id.toString());
  console.log(hashString);

  thread._update.$push.comments.seq_id = hashString.slice(0, 8);
