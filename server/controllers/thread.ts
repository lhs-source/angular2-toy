...
export default class ThreadCtrl extends BaseCtrl {
    ...
    countThread = (req, res) => {
        ...
        this.model.count({category : req.params.category}, (err, count) => {
            if (err) { return console.error(err); }
            console.log(count);
            res.json(count);
        });
    ...
    insertComment = (req, res) => {
        ...
        // pre
        let id = obj.comments[0]._id;
        let hashString = SHA256(id.toString());
        obj.comments[0]['seq_id'] = hashString.slice(0, 8);
        ...
        this.model.findOneAndUpdate(condition, pipe, { safe: true, upsert: true }, (err, item) => {
            ...
            let responseObject = {
                comments : {
                    _id : obj.comments[0]._id,
                    seq_id : obj.comments[0].seq_id,
                }
            }
    ...
    getComment = (req, res) => {
        ...
        this.model.findOne(condition, pipe, (err, obj) => {
            if (err) { return console.error(err); }
            res.json(obj);
        });
        console.log("get");
    ...
    updateComment = (req, res) => {
        ...
        // update할 때 update인지 delete인지에 따라 pipe 설정
        ...
        this.model.update(condition, pipe, (err, item) => {
            if (err) { return console.error(err); }
            console.log(item);
            let responseObject = {
                comments : {
